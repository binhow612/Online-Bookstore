...
export default async function ProductPage({
  ...
  // 1. Fetch dữ liệu sách, categories, reviews và order items
  ...
  // 2. Tính toán dữ liệu thực tế
  // --- Tính Rating trung bình ---
  const reviewCount = book.bookReviews.length;
  const averageRating =
    reviewCount > 0
      ? book.bookReviews.reduce((acc: number, review) => acc + review.rating, 0) /
        reviewCount
      : 0;
  const ratingDisplay = averageRating > 0 ? averageRating.toFixed(1) : "Chưa có";

  // --- Tính số lượng đã bán ---
  // Thêm type (acc: number) để tránh lỗi "implicitly has an 'any' type"
  const soldCount = book.bookOrderItems.reduce((acc: number, item) => acc + item.quantity, 0);
  ...
  // --- Danh mục ---
  const categoryNames = book.bookCategories
      ...
      <div className="container mx-auto pt-6">
              ...
              <div className="space-y-6 pl-2">
                <div className="flex gap-4 items-center text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[#4E3B31]/50 text-xs">
                      Đổi ý miễn phí
                    </span>
                  </div>
                </div>

                {/* Add To Cart Component (Bao gồm cả chọn số lượng) */}
        ...
        <div className="bg-[#FBF8F3] rounded-xl shadow-[0_6px_18px_rgba(78,59,49,0.06)] p-6 mt-6 border border-[#4E3B31]/10">
          <div className="bg-[#F5EDE3] p-3 mb-6 rounded-md">
            <h2 className="text-lg font-bold text-[#8B6B4F] uppercase">
              Chi tiết sản phẩm
            </h2>
          </div>
          <div className="space-y-4 px-4 text-sm text-[#4E3B31]">
            <div className="grid grid-cols-12 gap-4">
              <div className="col-span-4 md:col-span-2 text-[#4E3B31]/70">
                Danh Mục
              </div>
              <div className="col-span-8 md:col-span-10 text-[#8B6B4F] font-medium font-serif">
                {categoryNames || "Sách"}
              </div>
            </div>
            <div className="grid grid-cols-12 gap-4">
            ...
            <div className="grid grid-cols-12 gap-4">
              ...
              <div className="col-span-8 md:col-span-10">
                {book.publication_year || "Đang cập nhật"}
              </div>
            </div>
             <div className="grid grid-cols-12 gap-4">
              <div className="col-span-4 md:col-span-2 text-[#4E3B31]/70">
